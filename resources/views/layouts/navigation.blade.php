<nav
    class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex justify-end items-center gap-4">
    <!-- Settings Dropdown -->
    ...
    <div x-data="{ darkMode: localStorage.getItem('theme') === 'dark' }" x-init="document.documentElement.classList.toggle('dark', darkMode)" class="hidden sm:flex px-4 py-2">
        <label class="flex items-center gap-2 cursor-pointer text-sm">
            <!-- Dynamic Icon -->
            <span class="text-xl" x-text="darkMode ? '🌙' : '☀️'" class="text-gray-700 dark:text-gray-200"></span>

            <!-- Toggle Switch -->
            <div class="relative">
                <input type="checkbox" class="sr-only" x-model="darkMode"
                    @change="localStorage.setItem('theme', darkMode ? 'dark' : 'light'); document.documentElement.classList.toggle('dark', darkMode)">
